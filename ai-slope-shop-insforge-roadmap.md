...
Статус: backend-миграция завершена и работает в production; остались проверка прав anon-доступа и диагностика заголовков Storage.

Текущий следующий шаг: **проверить, что anon может только читать каталог, и зафиксировать характеристики выдачи Storage**.
...
На стороне InsForge для диагностики нужно:
...
Для закрытия диагностики повторить запрос из DevTools Network или терминала и сохранить фактические `Cache-Control`, `ETag`, `Last-Modified`, `Content-Type`, `Content-Length` и время ответа для нескольких объектов.
