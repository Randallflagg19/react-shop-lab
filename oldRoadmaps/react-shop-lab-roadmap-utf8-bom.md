...
Дни 1–21 завершены в базовой версии. Дальнейшие продуктовые улучшения ведутся в `ai-slope-shop-frontend-roadmap.md`, а backend-проверки — в `ai-slope-shop-insforge-roadmap.md`.
...
Итог основной учебной программы:

> Favorites используют внешний store и `useSyncExternalStore`, корзина сохраняет `useReducer` + Context, оба состояния переживают reload и синхронизируются между вкладками. Эксперимент `useOptimistic` завершён и удалён из продуктового кода как ненужный для синхронного `localStorage`. README, deployment и корневые metadata подготовлены.

Замечание по API:

> Первоначальный Platzi Fake Store API заменён собственным каталогом в InsForge Database и Storage. История ранних этапов ниже сохранена как учебный контекст.
...
- favorites вынесены во внешний `favoritesStore` и подключены через `useSyncExternalStore`;
- реализованы стабильные client/server snapshots и подписка текущей вкладки;
- favorites сохраняются в `localStorage` и синхронизируются между вкладками;
- корзина сохранила архитектуру `useReducer` + Context, восстанавливается после reload и синхронизируется между вкладками;
- browser-only API не читаются во время серверного рендера.
...
Статус: **завершён как учебный эксперимент; код удалён из продуктовой версии**.

Вывод:

> Optimistic UI нужен, когда интерфейс заранее показывает ожидаемый результат удалённой асинхронной операции и должен уметь откатиться при ошибке. В текущих favorites нет серверного запроса: переключение синхронно записывается в `localStorage` и сразу уведомляет внешний store. Поэтому `useOptimistic`, `startTransition` и искусственная задержка не ускоряли реальную операцию, а создавали лишнее состояние и визуальное мигание.

Эксперимент помог разобрать optimistic update, pending state и rollback, после чего был осознанно удалён. Если favorites позднее переедут на сервер, концепцию можно вернуть вокруг настоящего запроса.
...
- README описывает актуальный стек, InsForge backend, маршруты, hooks, архитектуру и ограничения;
- добавлены скриншоты каталога;
- приложение развёрнуто на Vercel;
- корневой layout экспортирует статические Metadata API поля: title, title template, description и application name;
- список дальнейших улучшений сохранён в frontend и InsForge roadmap.

Что остаётся необязательной полировкой, а не блокером Дня 21:

- добавить динамический title для страницы конкретного товара через `generateMetadata`;
- обновлять скриншоты после заметных визуальных изменений;
- дополнить README ссылкой на production deployment, если нужен публичный URL;
- поддерживать раздел ограничений синхронно с frontend-roadmap.

## Цель

Довести проект до состояния учебного showcase.

## Что сделать — выполнено
